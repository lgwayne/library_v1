...
public class LendDao {
    ...
    public ArrayList<Lend> myLendList(final long reader_id,int page,int size) {
        PageHelper.startPage(page,size);
    ...
    public ArrayList<String> getBookName(long readerId) {
        List<String> result =sqlSessionTemplate.selectList(NAMESPACE+"mylend_bookName",readerId);
        return (ArrayList<String>) result;
    }

    public boolean deleteLendByBookId(long bookId) {
        return  sqlSessionTemplate.delete(NAMESPACE + "deleteLendByBookId", bookId)>0;
    }
}
